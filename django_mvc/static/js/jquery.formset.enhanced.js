    ...
    $.fn.formset = function(opts)
    {
        ...
        if ($$.length) {
            ...
            if ($$.is('TR')) {
                ...
                var $formsetParent = $$.parent();
                //table can have footer, the 'Add Another' link should append after the table footer.
                var $table = $$.parent();
                while (!$table.is("TABLE")) {
                    $table = $table.parent()
                }
                if ($table.children("tfoot").length) {
                    $table.children("tfoot").children("tr:last").parent().append(buttonRow);
                } else if ($table.children("tbody").length) {
                    $table.children("tbody").children("tr:last").parent().append(buttonRow);
                } else {
                    $table.children("tr:last").parent().append(buttonRow);
                }
                if (hideAddButton) buttonRow.hide();

                addButton = buttonRow.find('a');
                var addNewRow = null;
                if ($$.parent().is($table.find("tr:last").parent())) {
                    //"Add Another" link has the same parent as formset body
                    addNewRow = function(row) {
                        row.insertBefore(buttonRow).show();
                    }
                } else {
                    //"Add Another" link has different parent as formset body
                    addNewRow = function(row) {
                        $$.parent().append(row)
                        row.show()
                    }
